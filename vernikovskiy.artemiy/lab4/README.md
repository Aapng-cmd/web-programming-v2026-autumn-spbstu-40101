...
Пользовательский интерфейс находится в `index.html` и подкреплён `main.js` и `styles.css`.
* Я остановился на варианте "ввода" предмета, а не выбора, потому что мне так удобнее.
* Если студента с нужным id нет, то оценка не добавится выдав alert "Студент не найден".
* Так как нигде не было сказано реализовать функции из 4.2 в пользовательском интерфейсе, никаких кнопок с подвязкой к этим функциям нет. Студенты отображаются в порядке их добавления.
* Предмет можно "пересдать", то есть для каждой комбинации "студент + предмет" оценка единственная.
